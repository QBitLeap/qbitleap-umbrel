...
class StratumRouterTests(unittest.TestCase):
    def test_rewrites_worker_authorization_to_qbit_payout(self):
        with tempfile.TemporaryDirectory() as directory:
            address_file = Path(directory) / "miner_address"
            address_file.write_text("qb1validpayout\n", encoding="utf-8")
            payload = json.dumps({"id": 2, "method": "mining.authorize", "params": ["thor-p2", "x"]}).encode() + b"\n"
            with patch.object(router, "MINER_ADDRESS_FILE", address_file):
                remaining, output = router.rewrite_miner_messages(b"", payload)
    ...
    def test_preserves_partial_stratum_messages(self):
        remaining, output = router.rewrite_miner_messages(b"", b'{"id":1')
